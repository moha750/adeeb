-- ٢٠٢٦-٠٩-٠٥ — تسميةُ الدخول المباشر تُفصح عن نفسها، والذيلُ يُطوى بعد الخامس والعشرين.
--
-- «دخولٌ مباشر» لا يقول ما هو، فصار «دخولٌ مباشر من رابط الموقع»: من كتب العنوان أو فتح
-- رابطًا محفوظًا. والسقفُ رُفع من ١٢ إلى ٢٥ لأنّ العرضَ صار قائمةً لا حلقةً محدودةَ الألوان.
...
as $function$
  ...
  select jsonb_build_object(
    ...
    -- قائمةُ النطاقات الخام: تُنخل منها ما ليس مصدرًا (إحالةُ الموقع لنفسه وبوّاباتُ الدخول
    -- وخادمُ التطوير) — كانت `adeeb.club` تتصدّرها بـ١١٤٢ إحالةً وهي تنقّلٌ داخليّ لا مصدر.
    'referrers', (select coalesce(jsonb_agg(jsonb_build_object('label', referrer_host, 'count', c) order by c desc), '[]'::jsonb)
                  from (select referrer_host, count(*) c from base
                        where nullif(referrer_host, '') is not null
                          and referrer_host !~* 'adeeb\.club'
                          and referrer_host !~* '(accounts\.google\.com|appleid\.apple\.com|supabase\.co)'
                          and referrer_host !~* '(^localhost|^127\.|^192\.168\.)'
                        group by 1 order by count(*) desc limit 10) t),
    ...
    'recent', (select coalesce(jsonb_agg(jsonb_build_object(
               ...
               from (select visitor_id vid,
                     ...
                     limit 12) t),
    -- ٤ إحصاءاتٍ أُقِرّت بعد معاينة `/ui/analytics-next` (٢٠٢٦-٠٩-٠٥):

    -- (١) صفحاتُ الدخول: أوّلُ صفحةٍ في كلّ جلسة — نظيرةُ صفحات الخروج ومقلوبُها.
    'entry_pages', (select coalesce(jsonb_agg(jsonb_build_object('label', page_path, 'title', ttl, 'count', c) order by c desc), '[]'::jsonb)
                    from (select page_path,
                                 (array_agg(page_title order by visited_at desc)
                                    filter (where page_title is not null and page_title <> ''))[1] ttl,
                                 count(*) c
                          from (select distinct on (session_id) session_id, page_path, page_title, visited_at
                                ...
                                order by session_id, visited_at asc) first_pv
                          group by 1 order by count(*) desc limit 40) t),

    -- (٢) المصادرُ **بأسمائها كلُّها لا قائمةً مغلقة** (٢٠٢٦-٠٩-٠٥): كانت القائمةُ محدّدةً بيدي،
    -- فمن جاء من موقعٍ لم أسمِّه يقع في «مواقعُ أخرى» حتى أضيفه — صيانةٌ دائمةٌ وخبرٌ ضائع.
    -- فصار المجهولُ يظهر **باسم نطاقه** منزوعَ الزوائد (‏reddit.com · kfu.edu.sa)، ولا يبقى
    -- التحديدُ إلّا لدمج أسماء المنصّة الواحدة (l.instagram.com و instagram.com = Instagram).
    -- والذيلُ الطويل يُجمع في «مواقعُ أخرى» بعد الثاني عشر، فلا تنفجر الحلقةُ بشرائحَ لا تُقرأ.
    -- و**ثلاثةٌ تُنخل** لأنّها ليست مصدرًا: التنقّلُ داخل الموقع (adeeb.club يُحيل إلى نفسه)،
    -- والعودةُ من بوّابة تسجيل الدخول (accounts.google · appleid.apple)، وخادمُ التطوير.
    'sources_class', (select coalesce(jsonb_agg(jsonb_build_object('label', nm, 'count', c) order by c desc), '[]'::jsonb)
                      from (select case when rn <= 25 then nm else 'مواقعُ أخرى' end nm, sum(c) c
                            from (select nm, c, row_number() over (order by c desc) rn
                                  from (select case
                                          when h = '' then 'دخولٌ مباشر من رابط الموقع'
                                          when h ~* '(^|\.)t\.co$|twitter|x\.com'          then 'X (تويتر)'
                                          when h ~* 'instagram'                              then 'Instagram'
                                          when h ~* 'snapchat'                               then 'Snapchat'
                                          when h ~* 'tiktok'                                 then 'TikTok'
                                          when h ~* 'facebook'                               then 'Facebook'
                                          when h ~* 'linkedin'                               then 'LinkedIn'
                                          when h ~* '(whatsapp|wa\.me)'                      then 'WhatsApp'
                                          when h ~* '(t\.me|telegram)'                       then 'Telegram'
                                          when h ~* 'com\.google\.android\.gm'              then 'بريد Gmail'
                                          when h ~* '(google|googlequicksearchbox)'          then 'بحث Google'
                                          when h ~* 'bing'                                   then 'بحث Bing'
                                          when h ~* '(yahoo|duckduckgo|yandex|ecosia)'        then 'محرّكاتُ بحثٍ أخرى'
                                          -- وما سوى المعروف: **اسمُ نطاقه كما هو**، منزوعَ الزوائد
                                          else regexp_replace(
                                                 regexp_replace(
                                                   regexp_replace(h, '^android-app://', ''),
                                                 '^www\.', ''),
                                               '(:[0-9]+)?/*$', '')
                                        end nm,
                                        count(*) c
                                        from (select coalesce(referrer_host, '') h from base) r
                                        where h !~* 'adeeb\.club'
                                          and h !~* '(accounts\.google\.com|appleid\.apple\.com|supabase\.co)'
                                          and h !~* '(^localhost|^127\.|^192\.168\.)'
                                        group by 1) g) x
                            group by 1) t),

    -- (٣) مدّةُ الزيارة كلِّها لا الصفحةِ الواحدة: مجموعُ ثواني صفحات الجلسة. ومعها **الوسيط**،
    -- لأنّ حفنةً من الجلسات الطويلة ترفع المتوسّطَ فيكذب على الحال العامّ (٣د١٩ث متوسّطًا مقابل
    -- ٢ث وسيطًا في أوّل قياس، وسببُه أنّ أكثر الجلسات لم تُكتب لها مدّةٌ قبل إصلاح النبضة).
    'visit_seconds', (select jsonb_build_object(
        'avg',      coalesce(round(avg(secs)), 0),
        'median',   coalesce(round(percentile_cont(0.5) within group (order by secs)), 0),
        'sessions', count(*))
      from (select session_id, sum(total_seconds) secs from base where session_id is not null group by 1) s),

    -- توزيعُ الزيارات على خمس مُدَد (٢٠٢٦-٠٩-٠٥، اختيارُ المالك «مدرّج»): رقمٌ واحدٌ للمدّة
    -- يكذب (متوسّطٌ ٣د١٩ث ووسيطٌ ٢ث في القياس نفسِه)، والتوزيعُ يقول الحالَ كما هو.
    'visit_buckets', (select jsonb_build_array(
        jsonb_build_object('label', 'أقلّ من 10 ثوانٍ',      'count', count(*) filter (where secs < 10)),
        jsonb_build_object('label', 'من 10 ثوانٍ إلى دقيقة', 'count', count(*) filter (where secs >= 10  and secs < 60)),
        jsonb_build_object('label', 'من دقيقة إلى 5 دقائق',  'count', count(*) filter (where secs >= 60  and secs < 300)),
        jsonb_build_object('label', 'من 5 إلى 15 دقيقة',     'count', count(*) filter (where secs >= 300 and secs < 900)),
        jsonb_build_object('label', 'أكثر من 15 دقيقة',      'count', count(*) filter (where secs >= 900)))
      from (select session_id, sum(total_seconds) secs from base where session_id is not null group by 1) s),

    -- (٤) أطولُ الصفحات مكثًا: متوسّطُ البقاء في الصفحة الواحدة. تُستبعَد الأصفار (خروجٌ قبل
    -- أوّل نبضة) وما دون خمسِ مشاهداتٍ (صفحةٌ فُتحت مرّةً بمصادفةٍ ليست خبرًا). والثواني في
    -- خانة `count` كي تبقى الصورةُ صورةَ قائمةٍ واحدةً في كلّ الأقسام.
    'dwell_pages', (select coalesce(jsonb_agg(jsonb_build_object('label', page_path, 'title', ttl, 'count', secs) order by secs desc), '[]'::jsonb)
                    from (select page_path,
                                 (array_agg(page_title order by visited_at desc)
                                    filter (where page_title is not null and page_title <> ''))[1] ttl,
                                 round(avg(total_seconds))::int secs
                          from base where total_seconds > 0
                          group by 1 having count(*) >= 5
                          order by avg(total_seconds) desc limit 20) t),
