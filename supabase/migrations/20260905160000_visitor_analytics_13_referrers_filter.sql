-- ٢٠٢٦-٠٩-٠٥ — قائمةُ النطاقات الخام تُنخل كما نُخل تصنيفُها.
--
-- كانت `adeeb.club` تتصدّر «أعلى مصادر الإحالة» بـ١١٤٢ إحالة، وهي تنقّلٌ داخل الموقع لا مصدرٌ
-- خارجيّ. فصار النخلُ واحدًا في الموضعين: الاسمُ المصنَّف والنطاقُ الخام.
...
as $function$
  ...
  select jsonb_build_object(
    ...
    -- (٢) المصادرُ **بأسمائها** (٢٠٢٦-٠٩-٠٥): «تواصلٌ اجتماعيّ» جوابٌ لا يُبنى عليه عمل — أيُّ
    -- منصّةٍ تحديدًا؟ فصار لكلّ منصّةٍ اسمُها (X · Instagram · TikTok · Snapchat …)، والنطاقاتُ
    -- المختلفةُ للمنصّة الواحدة تجتمع تحت اسمها (l.instagram.com و instagram.com سواء).
    ...
    'sources_class', (select coalesce(jsonb_agg(jsonb_build_object('label', nm, 'count', c) order by c desc), '[]'::jsonb)
                      from (select case
                              ...
                              when h ~* '(bing|yahoo|duckduckgo|yandex|ecosia)'    then 'محرّكاتُ بحثٍ أخرى'
                              else 'مواقعُ أخرى' end nm,
                            count(*) c
