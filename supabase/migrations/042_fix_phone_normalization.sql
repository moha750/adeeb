-- ============================================================================
-- إصلاح مشكلة التحقق من رقم الهاتف: توحيد الصيغة (Normalization)
-- Migration 042
-- ============================================================================

-- ============================================================================
-- 1. إنشاء دالة لتوحيد صيغة رقم الهاتف
-- ============================================================================
...
-- ============================================================================
-- 2. تحديث دالة validate_phone_for_booking لاستخدام التوحيد
-- ============================================================================
...
BEGIN
    ...
    -- نستخدم normalize_phone على الأرقام المخزنة أيضاً للمقارنة
...
-- ============================================================================
-- 3. إنشاء index لتسريع البحث بالرقم الموحد
-- ============================================================================

-- إنشاء index وظيفي على normalize_phone(phone)
...
'توحيد صيغة رقم الهاتف السعودي: إزالة المسافات والرموز، معالجة كود الدولة، التأكد من الصيغة الصحيحة';

COMMENT ON FUNCTION validate_phone_for_booking(TEXT, UUID) IS 
'التحقق من صحة رقم الهاتف للحجز مع توحيد الصيغة تلقائياً - يعمل على جميع المتصفحات والأجهزة';
...
DO $$
BEGIN
    RAISE NOTICE '✅ تم إصلاح مشكلة التحقق من رقم الهاتف';
    RAISE NOTICE '📌 التحسينات:';
    RAISE NOTICE '   - توحيد صيغة الأرقام تلقائياً';
    RAISE NOTICE '   - إزالة المسافات والرموز';
    RAISE NOTICE '   - معالجة كود الدولة (+966)';
    RAISE NOTICE '   - يعمل على جميع المتصفحات والأجهزة';
    RAISE NOTICE '   - تحسين الأداء بإضافة index';
END $$;
