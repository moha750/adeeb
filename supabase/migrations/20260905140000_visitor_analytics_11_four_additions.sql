-- ٢٠٢٦-٠٩-٠٥ — أربعُ إحصاءاتٍ جديدة: صفحاتُ الدخول · تصنيفُ المصادر · مدّةُ الزيارة · أطولُ مكثًا.
--
-- عُرِضت على المالك لقطةً حيّةً في `/ui/analytics-next` فأقرّها كلَّها. وكلُّها تُحسب من `base`
-- نفسِها، فترث المدى والبابَ ونخلَ الروبوتات وصفحاتِ الإدارة بلا شرطٍ يُعاد كتابتُه.
--
...
as $function$
  ...
  select jsonb_build_object(
    ...
    -- (٢) تصنيفُ المصادر: أربعةُ أصنافٍ تُقرأ بدل أسماء نطاقاتٍ خام. و**ثلاثةٌ تُنخل** لأنّها
    -- ليست مصدرًا أصلًا: التنقّلُ داخل الموقع (adeeb.club يُحيل إلى نفسه)، والعودةُ من بوّابة
    -- تسجيل الدخول (accounts.google.com · appleid.apple.com)، وخادمُ التطوير المحلّيّ.
    'sources_class', (select coalesce(jsonb_agg(jsonb_build_object('label', nm, 'count', c) order by c desc), '[]'::jsonb)
                      from (select case
                              when h = '' then 'دخولٌ مباشر'
                              when h ~* '(google|bing|yahoo|duckduckgo|yandex|ecosia|baidu)' then 'محرّكات البحث'
                              when h ~* '(^t\.co$|twitter|x\.com|instagram|facebook|^fb\.|snapchat|tiktok|linkedin|t\.me|telegram|whatsapp|threads)' then 'تواصلٌ اجتماعيّ'
