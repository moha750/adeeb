...
-- النسخة: 20260815173230   الاسم: volunteer_opportunity_optional_start

-- تاريخُ البداية اختياريٌّ (قرار المالك ١٥ أغسطس ٢٠٢٦): تُفتح الفرصةُ ويُنشر رابطُها قبل أن
-- يُحسم موعدُها، فلا يُحبَس فتحُها على تاريخٍ لم يُتّفق عليه بعد.
alter table public.volunteer_opportunities alter column starts_on drop not null;

-- والنهايةُ لا تقوم بلا بداية: تاريخُ انتهاءٍ بلا ابتداءٍ جملةٌ ناقصة، والقيدُ يقولها
alter table public.volunteer_opportunities drop constraint if exists opportunity_dates;
...
-- والشهادةُ تحتاج تاريخًا تُكتب فيه: فإن لم يكن للفرصة بدايةٌ فيومُ الإصدار هو يومُ المشاركة
...
begin
  ...
  values
    (p_application_id, v_app.user_id, v_serial, v_name, v_gender, v_opp.title, v_committee,
     coalesce(v_opp.starts_on, v_today), v_opp.ends_on, v_actor)
