-- ٢٠٢٦-٠٩-٠٥ — توزيعُ مدّة الزيارة على خمس مُدَد، لمدرّجٍ يحلّ محلّ الأرقام الثلاثة.
...
as $function$
  ...
  select jsonb_build_object(
    ...
    'sources_class', (select coalesce(jsonb_agg(jsonb_build_object('label', nm, 'count', c) order by c desc), '[]'::jsonb)
                      from (select case when rn <= 12 then nm else 'مواقعُ أخرى' end nm, sum(c) c
                            from (select nm, c, row_number() over (order by c desc) rn
                                  from (select case
                                          when h = '' then 'دخولٌ مباشر'
