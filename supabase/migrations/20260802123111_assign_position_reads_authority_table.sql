...
-- النسخة: 20260802123111   الاسم: assign_position_reads_authority_table

-- الإسناد كما وصفه المالك: بلوغُ المقعد يكفي للسحب، وشرطُ المقعد يُفحَص، والترقيم يسقط.
--
-- سقط من هذه الدالّة ثلاثة أشياء صارت غير ذات معنى:
--   · بوّابةُ القدرة (`manage_positions` / `assign_unit_members`) — صار البابُ صفًّا في
--     `position_authority`. والقدرتان تبقيان مفتاحَي تبويبٍ في اللوحة لا حَكَمًا هنا.
--   · حارسُ الرتبة العدديّة (`v_actor_rank > v_target_rank`) — الجدول يقول ما يبلغه كلُّ
--     دورٍ صراحةً، فلا حاجة لعتبةٍ رقميّة (ومبدأ الترقيم محظورٌ أصلًا).
--   · اشتراطُ بلوغ المنصب القديم — صار السحب تابعًا للإجلاس، والممنوعُ يُسمَّى.
...
begin
  ...
  --     فهو انتقالٌ من موضعٍ معلوم لا ضمٌّ من خارج.
  ...
  -- (5) الإخلاء: يُطفأ ما كان قبل أن يُكتَب ما صار — في المعاملة نفسها، فلا يمرّ العضو
  --     بلحظةٍ يحمل فيها منصبين. ويُسمّى المُخلى في الرسالة فلا يكون النزع صامتًا.
  with vac as (
