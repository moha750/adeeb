...
function initCategorySwiper(rootEl) {
  ...
  containers.forEach((el) => {
    ...
    const spv = (target) => Math.max(1, Math.min(slidesCount, target));
    const instance = new Swiper(el, {
      slidesPerView: spv(1),
      spaceBetween: 24,
      loop: slidesCount > 3,
      rewind: slidesCount <= 3,
      autoplay: prefersReduced || slidesCount <= 1 ? false : { delay: 3500, disableOnInteraction: false },
      speed: prefersReduced ? 0 : 550,
      watchOverflow: true,
      allowTouchMove: slidesCount > 1,
      breakpoints: {
        700: { slidesPerView: spv(2) },
        1024: { slidesPerView: spv(3) },
      },
    ...
    if (slidesCount <= 1) {
      ...
      const navWrap = el.querySelector('.swiper-button');
      if (navWrap) navWrap.style.margin = '0';
    }
    // علّم كونه مُهياً
    el.dataset.inited = '1';
  ...
  try {
    ...
    try {
      ...
      if (sb && ids.length) {
        ...
        const [
          { data: likesRows, error: likesErr },
          { data: commentsRows, error: commentsErr },
          { data: viewsRows, error: viewsErr }
        ] = await Promise.all([
          sb.from('blog_likes').select('post_id').in('post_id', ids),
          sb.from('blog_comments').select('post_id').in('post_id', ids),
          sb.from('blog_views').select('post_id').in('post_id', ids),
        ]);
        ...
        if (!viewsErr && Array.isArray(viewsRows)) {
          ...
          viewsRows.forEach(r => {
            const pid = r.post_id; viewMap.set(pid, (viewMap.get(pid) || 0) + 1);
          });
          posts.forEach(p => { p.views = viewMap.get(p.id) || (typeof p.views === 'number' ? p.views : 0); });
        }
      }
    } catch {}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  (function initBlogHeroCtaAuthAware() {
    ...
    const setLoggedOut = () => {
      ...
      cta.setAttribute('href', '../blogger/register.html');
    };
