...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    async function refreshAuth() {
      try {
        if (!sb) { currentUser = null; isBlogger = false; isEditor = false; return; }
        ...
        currentUser = null; isBlogger = false; isEditor = false;
      }
    }
    ...
    function enforceCommentAccessUI() {
      ...
      if ((isBlogger || isEditor) && currentUser) {
    ...
    function updatePostSEO(postId, p) {
      try {
        const base = 'https://www.adeeb.club/marafi/blog/';
        const canonicalUrl = `${base}post.html?id=${encodeURIComponent(postId)}`;
    ...
    async function deleteCommentSB(cid) {
      if (!sb) return false;
      try {
    ...
    async function addViewSB(id, deviceId) {
      if (!sb) return false;
      try {
        const { error: rpcErr } = await sb.rpc('record_post_view', { pid: id, pdevice: deviceId });
      ...
      try {
        ...
        try {
          const { data: existing } = await sb
            ...
            .maybeSingle();
          if (existing) {
            const nextVisits = (Number(existing.visits) || 0) + 1;
            await sb
              .from('blog_views')
              .update({ visits: nextVisits, last_viewed_at: new Date().toISOString() })
              .eq('post_id', id)
              .eq('device_id', deviceId);
            return true;
          }
    ...
    async function trackView(id) {
      ...
      if (hasSeenRecentlyLS(id, 0)) { await refreshViewsUI(id); return false; }
      ...
      slice.forEach(c => {
        ...
        div.innerHTML = `
          <div class="comment-header">
            ...
            ${isEditor ? `<button class="comment-delete-btn" data-cid="${c.id}" title="حذف"><i class="fa-solid fa-trash"></i></button>` : ''}
          </div>
    ...
    if (el.commentsList) {
      el.commentsList.addEventListener('click', async (e) => {
        ...
        if (del) {
          if (!isEditor) return;
          const cidDel = del.getAttribute('data-cid');
          if (!cidDel) return;
          if (!confirm('حذف هذا التعليق؟')) return;
          const ok = await deleteCommentSB(cidDel);
          if (ok) { await loadComments(postId); }
          return;
        }
    ...
    if (el.commentForm) {
      el.commentForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!(isBlogger || isEditor)) { if (el.commentStatus) el.commentStatus.textContent = 'التعليقات متاحة للمدونين والمحررين فقط.'; return; }
