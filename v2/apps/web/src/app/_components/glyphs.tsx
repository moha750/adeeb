...
/**
 * **ثقبٌ في المنطقة** — سطحٌ داخل `DuotoneZone` يعلن أنّ ما فيه يبقى على وزن الاستثناء.
 *
 * والعلّةُ أنّ المنطقةَ تُعلَن على الحاوية فتعمّ ذرّيّتَها كلَّها، وفي الحاوية أحيانًا
 * ما ليس منها: **زرُّ إغلاقٍ ليس بندَ تنقّل**. فأوّلُ ساكنيه أيقونةُ الإغلاق في ورقة
 * التنقّل (أمرُ المالك ٢٠٢٦-٠٨-٢٣: «أستثني أيقونة أكس في `ash-side mnsh`») — طبقةُ `×`
 * الخافتة **مربّعٌ مستدير يملأ الإطار** لا شكلُ الرمز، وهو أوّلُ ما يفسده duotone.
 *
 * ويبقى القانونُ واحدًا: الاستثناءُ **بالمكان لا بالخاصّة** — يُعلَن على السطح ولا
 * يُمرَّر وزنٌ في وسمٍ ولا يُستورَد اسمٌ من Phosphor التفافًا على القائمة.
 */
export function GlyphZone({ children }: { children: React.ReactNode }) {
  return <DuotoneZoneCtx.Provider value={false}>{children}</DuotoneZoneCtx.Provider>;
}
