...
/**
 * **يُطبّع المفتاح مهما وصل** — ويعيد بناءه PEM قانونيًّا لا يردّ `\n` وحدها.
 *
 * **ولماذا هذا القدر؟** لأنّ المفتاح يمرّ بأيدٍ ولوحاتٍ قبل أن يبلغنا: يُلصَق في حقل
 * Vercel، ويُنسَخ من ملفّ JSON، ويُكتب في `.env.local`. وفي كلّ ممرٍّ قد تُفقَد أسطرُه أو
 * تُبدَّل بمسافات أو تُكتب `\n` حرفين أو يُلفّ بعلامتَي اقتباس. وكلُّ صورةٍ من هذه تُخرج
 * من OpenSSL خطأً واحدًا غامضًا: `DECODER routines::unsupported` — لا يقول أيَّ شيءٍ عن
 * السبب. (وقع فعلًا عند أوّل رفعٍ إلى الإنتاج ٢٠٢٦-٠٨-٠٤.)
 *
 * **فالعلاجُ ألّا نشترط صورةً بعينها**: نأخذ الترويسةَ والذيلَ وما بينهما، ونُسقط كلَّ
 * فراغٍ من الجسم، ثمّ نلفّه ٦٤ محرفًا كما يقتضي المعيار. فالمفتاحُ الصحيح يُقرأ بأيّ
 * صورةٍ وصل، والفاسدُ وحده يبقى فاسدًا.
 */
const pem = (v: string): string => {
  const raw = v
    .trim()
    .replace(/^["']|["']$/g, "") // اقتباسٌ لفّه أحدُهم
    .replace(/\\n/g, "\n") // `\n` كُتبت حرفين
    .replace(/\r/g, ""); // أسطرُ ويندوز

  const m = raw.match(/-----BEGIN ([A-Z ]+)-----([\s\S]*?)-----END \1-----/);
  if (!m) return raw; // ليس PEM — يُترَك كما هو فيقول OpenSSL كلمتَه
...
export async function patchObject(m: DemoMember, mode: Mode, origin: string): Promise<GoogleSync> {
  ...
  /**
   * **وهذا ما يُشعِر — لا الرسالة.** جُرّب على جهازٍ حقيقيّ (٢٠٢٦-٠٨-٠٧): البطاقةُ تتحدّث
   * بـ`PATCH` ناجحٍ (٢٠٠) ولا يصل إشعار. فقوقل تُحدّث صامتةً ما لم يُطلَب التنبيه صراحةً.
   *
   * **وهو حقلٌ عابر** (`transient`): لا يُخزَّن في الكائن، فيُعاد إرسالُه مع كلّ نداءٍ
   * يُراد أن يُشعِر — ولذلك يُكتب هنا في `patchObject` لا في `loyaltyObject`.
   *
   * **ولا يُشعِر تغييرُ أيّ حقل**: قوقل تحصر ذلك في `loyaltyPoints.balance` و
   * `secondaryLoyaltyPoints.balance` من الكائن (و`programName` وطبقتَي المكافآت من
   * الصنف). **وعدّادُنا هو `loyaltyPoints.balance` بعينه** — فما نغيّره هو ما يُشعِر.
   *
   * > **وسقفٌ يُعلَن**: ثلاثةُ إشعاراتٍ لكلّ بطاقةٍ في أربعٍ وعشرين ساعة. فمن جرّب أكثر
   * > من ثلاثِ ختماتٍ متتابعةٍ رأى البطاقةَ تتحدّث والإشعارَ يصمت — وليس ذلك عطلًا.
   */
  body.notifyPreference = "NOTIFY_ON_UPDATE";

  // والرسالةُ تُكتب في ظهر البطاقة (نظيرةُ حقل «آخر تحديث» عند أبل) — لا لتُشعِر
  body.messages = [
    {
      header: "نادي أَدِيب",
      body: points
