/**
 * بيانات المعاينة — **وهميّةٌ كلُّها**، لا صفٌّ منها من القاعدة ولا اتّصال بها.
 *
 * والنموذج **بطاقةُ أختام** لا نظامَ نقاط: عشرُ مشاركات، وعند العاشرة تُصرَف المكافأة
 * ويعود العدّاد صفرًا. لا رتبَ ولا أرصدةَ تُنفَق ولا عروضَ متفاوتة — قرارُ المالك بعد
 * أن رأى الأوّل مبالغًا فيه.
 */

/** عدد المشاركات التي تكتمل بها البطاقة. */
export const GOAL = 10;

/* ── المكافأة ───────────────────────────────────────────────────────────── */

/**
 * مكافأةٌ **واحدةٌ للنادي كلّه** يُتّفق عليها مع الراعي — لا قائمةَ عروضٍ تُفاضَل
 * بينها. تتغيّر بتغيّر الراعي، فهي هنا سطرٌ واحد.
 *
 * والراعي **مُختلَق** — لا نضع اسم جهةٍ حقيقيّة في عرضٍ لم تتّفق عليه.
 */
export const REWARD = {
  sponsor: "مقهى «سَطْر»",
  title: "مشروبٌ مجّانيّ من اختيارك",
  terms: "يُستلَم من الفرع بعرض البطاقة · لمرّةٍ واحدة لكلّ بطاقةٍ مكتملة",
};

/* ── الأعضاء ────────────────────────────────────────────────────────────── */

export type DemoMember = {
  id: string;
  name: string;
  gender: "male" | "female";
  /**
   * القسم واللجنة — يُرسَمان على البطاقة كما طلب المالك، و**من الهيكلة الحقيقيّة**
   * (`departments` · `committees`) لا من الخيال: البيانات وهميّةٌ والأسماءُ ليست.
   *
   * **وبلا لفظَي «قسم» و«لجنة»**: التسميةُ فوق القيمة تقولهما (`القسم` · `اللجنة`)،
   * فكتابتُهما في القيمة تُقرأ «القسم: قسم التواصل المجتمعي».
   */
  department: string;
  committee: string;
  /** المشاركات في الدورة الجارية (0…GOAL). */
  stamps: number;
  /** كم بطاقةً أكملها من قبل — يقول للراعي إنّ الولاء يتكرّر. */
  cycles: number;
  /** رقم البطاقة — المرسوم في الباركود والمقروء بالعين. */
  serial: string;
};

/**
 * أربعةُ حساباتٍ وهميّة تغطّي حالات البطاقة الأربع: **مكتملةٌ تنتظر الاستلام** ·
 * قاربت · في أوّلها · وفارغةٌ لم تبدأ.
 */
export const MEMBERS: DemoMember[] = [
  {
    id: "m1",
    name: "لمى صالح الدوسري",
    gender: "female",
    department: "التواصل المجتمعي",
    committee: "السُفراء",
    stamps: 10,
    cycles: 2,
    serial: "ADEEB-CARD-2026-0117",
  },
  {
    id: "m2",
    name: "عبدالله أحمد باجعيفر",
    gender: "male",
    department: "الإنتاج الإعلامي",
    committee: "التصوير",
    stamps: 7,
    cycles: 1,
    serial: "ADEEB-CARD-2026-0233",
  },
  {
    id: "m3",
    name: "ريّان محمد الحربي",
    gender: "male",
    department: "صناعة المحتوى",
    committee: "التأليف",
    stamps: 3,
    cycles: 0,
    serial: "ADEEB-CARD-2026-0341",
  },
  {
    id: "m4",
    name: "جُمانة فهد القحطاني",
    gender: "female",
    department: "نظم المعلومات",
    committee: "التقارير والأرشفة",
    stamps: 0,
    cycles: 0,
    serial: "ADEEB-CARD-2026-0402",
  },
];

/** عضوٌ بمعرّفه — أوّلُ القائمة إن لم يُطابِق شيء. */
export const memberById = (id: string): DemoMember => MEMBERS.find((m) => m.id === id) ?? MEMBERS[0];

/**
 * عضوٌ برقم بطاقته — **`null` إن لم يوجد**، بخلاف الذي قبله.
 * تسأله خدمةُ التحديث برقمٍ يأتي من الجهاز، فالمجهولُ يُردّ بـ٤٠٤ لا يُبدَّل بأوّل القائمة.
 */
export const memberBySerial = (serial: string): DemoMember | null =>
  MEMBERS.find((m) => m.serial === serial) ?? null;

/** هل اكتملت البطاقة فاستحقّت المكافأة؟ */
export const isComplete = (stamps: number): boolean => stamps >= GOAL;

/** حالةُ البطاقة سطرًا واحدًا — تُكتب على البطاقة وفي الصفحة، فلا صياغتان. */
export function statusText(stamps: number): string {
  if (isComplete(stamps)) return "مكافأةٌ جاهزة";
  const left = GOAL - stamps;
  if (left === 1) return "بقيت مشاركةٌ واحدة";
  if (left === 2) return "بقيت مشاركتان";
  return `بقيت ${num(left)} مشاركات`;
}

/* ── الصياغة ────────────────────────────────────────────────────────────── */

/**
 * رقمٌ بأرقامٍ **لاتينيّة** (قرار المالك ٢٠٢٦-٠٨-٠٤) — مصدرٌ واحدٌ لكلّ رقمٍ يظهر:
 * عدّادُ البطاقة، ونصُّ الحالة، ونصُّ الإشعار، والصفحة، وشاشةُ المسح. فلا يفترق رقمٌ
 * في الجيب عن نظيره على الشاشة.
 *
 * (كانت عربيّةً‑هنديّة `٤ / ١٠`؛ وتبديلُها هنا وحده بدّلها في المواضع كلّها.)
 */
export const num = (n: number): string => n.toLocaleString("en-US");

/**
 * العدّاد نصًّا (`9/10`) — **مصدرٌ واحدٌ للبطاقة والإشعار والصفحة وشاشة المسح**.
 *
 * **وبلا مسافتين حول الشرطة، والسببُ ليس ذوقًا**: في جملةٍ عربيّة تُرسَم الأرقام
 * يسارًا‑يمينًا، أمّا `/` **محاطةً بمسافتين** فتُقرأ محرفًا محايدًا يأخذ اتّجاه الجملة —
 * فينقلب ترتيبُ الجزأين ويظهر `10 / 9` بدل `9 / 10` (رآه المالك في المحفظة، وكان ظاهرًا
 * في اللقطات ولم يُلحَظ).
 *
 * وبإلصاقها تصير الشرطةُ **جزءًا من الرقم** بحكم خوارزميّة الاتّجاه نفسِها (قاعدة W4:
 * فاصلٌ مشتركٌ واحدٌ بين رقمين يصير رقمًا)، فتُرسَم الكتلةُ كلُّها يسارًا‑يمينًا ولا تنقلب.
 *
 * والعلاجُ بالبنية لا بمحارف تحكّمٍ خفيّة (LRI/PDI): تلك تُصلح المرئيّ وتُبقي النصّ
 * هشًّا — يُنسَخ فتُنسَخ معه، ويُقصّ فتضيع.
 */
export const score = (stamps: number): string => `${num(stamps)}/${num(GOAL)}`;
