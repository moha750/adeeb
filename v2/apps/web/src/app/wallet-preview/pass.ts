...
function stampsMessage(stamps: number): string {
  ...
  // **`%@` لا تُحذَف — قِيست على جهازٍ حقيقيّ.** جُرّبت هذه الرسالة مجرّدةً منها (طلبَها
  // المالك كذلك)، فلم يصمت الإشعار كما توقّعنا بل جاء **بعبارة أبل العامّة**: «تم تغيير
  // بطاقة المتجر». وهذا أسوأ من الصمت — نصٌّ باردٌ يحلّ محلّ صياغة صاحب البطاقة.
  //
  // فالحكم: `%@` **شرطُ اعتمادِ النصّ** لا شرطُ ظهور الإشعار. وأُعيدت هنا بكلمتين
  // تُبقيان جملة المالك كما كتبها وتجعلان الصفر بدايةً لا نقصًا.
  if (stamps === 0) return "بالعافية!😋 تم استلام المُكافأة وبطاقتك الجديدة تبدأ من %@";
