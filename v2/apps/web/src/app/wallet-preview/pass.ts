...
function stampsMessage(stamps: number): string {
  ...
  // **بلا `%@` عمدًا — وهي تجربةٌ لا سهو.** رسالةُ الاستلام لا موضعَ فيها لرقم: العدّاد
  // صار صفرًا، و«بطاقتك الجديدة 0» ركيكة. ومواصفةُ أبل تقول إنّ النصّ **يجب** أن يحمل
  // `%@`، فقد لا يظهر الإشعار أصلًا. تُقاس على جهازٍ حقيقيّ؛ فإن صمت رجعنا إلى صياغةٍ
  // تحمل الرقم في موضعٍ يُقرأ.
  if (stamps === 0) return "بالعافية!😋 تم استلام المُكافأة";
