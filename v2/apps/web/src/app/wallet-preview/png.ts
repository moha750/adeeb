 ...
 *
 * **والتدرّج يقف عند كحليّ‑700 لا كحليّ‑800**: الشريط بندٌ في أعلى البطاقة، وما تحته يرسمه
 * iOS بـ`backgroundColor` وهو كحليّ‑700. فلو أنهينا التدرّج عند كحليّ‑800 لَكانت أغمقُ
 * زاويةٍ فيه **أغمقَ ممّا تحتها** فيُقرأ خطُّ قصٍّ أفقيّ. وبالوقوف عند لون الجسم يصير
 * الجسمُ امتدادَ التدرّج لا لصقةً عليه.
 *
 ...
 * نملكها. فأُنزل إلى حافّة الشريط ليقع في المرئيّ منه — هو الضوءُ نفسُه، والمقتطَعُ تغيّر.
 */
function paintSurface(rgba: Buffer, w: number, h: number): void {
  ...
  for (let y = 0; y < h; y += 1) {
    for (let x = 0; x < w; x += 1) {
      ...
      const t = (x + y) / (w + h - 2);
      rgba[i] = Math.round(STEEL_400[0] + (NAVY_700[0] - STEEL_400[0]) * t);
      rgba[i + 1] = Math.round(STEEL_400[1] + (NAVY_700[1] - STEEL_400[1]) * t);
      rgba[i + 2] = Math.round(STEEL_400[2] + (NAVY_700[2] - STEEL_400[2]) * t);
      rgba[i + 3] = 255;
      ...
      if (d < 0.7) over(rgba, i, STEEL_300, 0.5 * (1 - d / 0.7));
    }
  }
}
...
function paintPattern(rgba: Buffer, w: number, h: number): void {
  ...
  for (let y = top; y < h; y += 1) {
    ...
    for (let x = 0; x < w; x += 1) {
      ...
      if (a > 0) over(rgba, (y * w + x) * 4, WHITE, a * 0.16);
    }
  }
}
