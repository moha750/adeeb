...
import { CORE_VERSION } from "@/lib/darb/core";
...
export async function POST(req: Request) {
  ...
  const { data, error } = await sb.rpc("darb_start_run", {
    p_token_hash: hash,
    p_user: user,
    p_seed: seed,
    p_core: CORE_VERSION,
  ...
  if (error) {
    if (error.message.includes("darb_no_player")) return fail(401, "اختر اسمَك أوّلًا.", { needName: true });
    if (error.message.includes("darb_too_many")) return fail(429, "جولاتٌ كثيرةٌ في وقتٍ قصير. خذ نفَسًا.");
