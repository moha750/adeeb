import { decodeInput, MAX_INPUT_CHARS, replay } from "@/lib/darb/rules";
import { service, who } from "@/lib/darb/player";
import { fail, isCount, isUuid, json, readJson, sameOrigin } from "@/lib/darb/http";
 ...
 * - `cap`: بلغت حدَّ الخادم حيّةً (ثلاثَ ساعات لعب، أي عمرَ التذكرة) ← تُحفَظ **ولا تدخل اللوحة** حتّى تُراجَع.
 *   ولا حدَّ للجولة في اللعبة نفسِها منذ ٢٠٢٦-٠٩-٢٦.
...
export async function POST(req: Request) {
  ...
  const { data: ticket, error: tErr } = await sb.rpc("darb_run_ticket", { p_token_hash: hash, p_user: user, p_run: body.run });
  ...
  const { data, error } = await sb.rpc("darb_finish_run", {
    p_token_hash: hash,
    p_user: user,
    p_run: body.run,
    p_ticks: r.ticks,
