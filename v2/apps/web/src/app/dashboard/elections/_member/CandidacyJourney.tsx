...
import { FileArrowDown, Note, Paperclip, Path, PencilSimple, Prohibit } from "@phosphor-icons/react";
...
export function CandidacyJourney({ c, onEdit, onWithdraw }: { c: CJ; onEdit: () => void; onWithdraw: () => void }) {
      ...
      <div className="cjr-body">
        ...
        <Card>
          ...
          <CardBody>
            ...
            {c.fileName ? (
              <FileButton block icon={<Paperclip />} label={c.fileName} hint="اضغط لتنزيل ملفّك" trailing={<FileArrowDown />} onClick={openFile} />
            ) : (
              <span className="cjr-nofile">لا ملفّ مرفق</span>
            )}
          </CardBody>
          {showControls ? (
