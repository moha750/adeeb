...
import {
  User, PencilSimple, Phone, Hash, Envelope, At, Lock, Key, IdentificationCard, CalendarBlank,
  Certificate, GraduationCap, Buildings, BookOpen, Books, GenderIntersex, Palette, XLogo, InstagramLogo, TiktokLogo, LinkedinLogo,
} from "@phosphor-icons/react";
import { DEGREES, DEGREE_VALUES, hasAcademicFields } from "@/lib/membershipFields";
import { onboardingSchema, emptyOnboarding, type OnboardingInput } from "./vocab";
import { completeOnboarding } from "./actions";

export type Prefill = { fullName: string; phone: string; email: string; degree: string; college: string; major: string };
...
export function OnboardingForm({ token, prefill }: { token: string; prefill: Prefill }) {
  ...
  const form = useForm<OnboardingInput>({
    ...
    defaultValues: {
      ...
      phone: prefill.phone,
      academic_degree: DEGREE_VALUES.includes(prefill.degree) ? prefill.degree : "",
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    startTransition(async () => {
      const res = await completeOnboarding(token, values);
      if (res.ok) {
        setDone(true);
        return;
      }
      if (res.fieldErrors) {
        for (const [key, message] of Object.entries(res.fieldErrors)) setError(key as keyof OnboardingInput, { message });
      }
    ...
    return (
      <Card tone="success" className="text-center">
        <CardBody className="p-8 md:p-10">
          ...
          <h2 className="font-display text-2xl font-black text-content">تم تفعيل حسابك 🎉</h2>
          <p className="mx-auto mt-2 max-w-md text-content-muted">أهلًا بك في عائلة أدِيب. سجّل دخولك للبدء.</p>
      ...
      <CardBody className="p-6 md:p-8">
        <form onSubmit={onSubmit} className="space-y-8" noValidate>
          ...
          <section className="space-y-4">
            <h3 className="font-display text-lg font-bold text-content">كلمة المرور</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="كلمة المرور" type="password" icon={<Lock />} innerIcon={<Key />} placeholder="٨ أحرف على الأقلّ" error={errors.password?.message} {...register("password")} required />
              <Field label="تأكيد كلمة المرور" type="password" icon={<Lock />} innerIcon={<Key />} placeholder="أعد كتابتها" error={errors.confirm?.message} {...register("confirm")} required />
            </div>
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="رقم الهويّة" charset="digits" icon={<IdentificationCard />} innerIcon={<Hash />} placeholder="١٠ أرقام" error={errors.national_id?.message} {...register("national_id")} required />
              <Field label="رقم الجوّال" type="tel" charset="digits" icon={<Phone />} innerIcon={<Hash />} placeholder="05xxxxxxxx" error={errors.phone?.message} {...register("phone")} required />
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {academic ? (
                <>
                  ...
                  <Field className="sm:col-span-2" label="الرقم الأكاديميّ" charset="digits" icon={<IdentificationCard />} innerIcon={<Hash />} placeholder="مثال: 443001234" error={errors.academic_record_number?.message} {...register("academic_record_number")} required />
                </>
              ) : null}
            </div>
          </section>
