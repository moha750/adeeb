 ...
 *
 * **والاستلامُ جملةٌ هنا وفعلٌ في «حسابي»:** لا رقمَ جوّالٍ يُجمع، فالفائزُ يُثبت اسمَه بحسابه أو
 * برمزه. وقراءةُ ذلك لا تحمي أحدًا، فالزرُّ يأخذه إلى حيث يؤمّنه.
 *
 * **والموعدُ من القاعدة** (`watan_contest_window`، محسوبًا في `page.tsx`) لا من هنا. فإن غاب
...
function Clock({ contest }: { contest: BoardContest }) {
  ...
  if (!contest) {
    return (
      <div className="wtna-card wtna-clock">
        <p className="wtna-clock-lbl">موعدُ المسابقة يُعلَن هنا قريبًا</p>
      </div>
    );
  }
  ...
  return (
    <div className="wtna-card wtna-clock" data-phase={phase}>
      <p className="wtna-clock-lbl">
        {phase === "before" ? "تبدأ المسابقة بعد" : phase === "open" ? "المسابقةُ جارية، وتنتهي بعد" : "انتهت المسابقة"}
      </p>
      {phase !== "after" ? (
        <div className="wtna-clock-row" role="timer" aria-live="off">
          {units.map((x) => (
            <span key={x.u} className="wtna-clock-u">
              <b>{x.v}</b>
              <span>{x.u}</span>
            </span>
          ))}
    ...
    <>
      ...
      <div className="wtna-body">
        ...
        <div className="wtna-card">
          ...
          <ol className="wtna-prizes">
            {RANKS.map(({ r, label }) => (
              <li key={r} className="wtna-prz" data-rank={r}>
                <span className="wtna-prz-rk">{r}</span>
                <b>{label}</b>
                <span className="wtna-prz-amt">{PRIZE[r]}</span>
              </li>
            ))}
          </ol>
        ...
        <div className="wtna-card">
          ...
          <button type="button" className="wtna-link" onClick={() => go("how")}>
        ...
        <div className="wtna-card">
          ...
          <p>
            ...
            أدِيب أو برمز الاسترجاع. فأمّن اسمَك قبل أن تفوز.
          </p>
          <button type="button" className="wtna-btn" data-wide="" onClick={() => go("me")}>
            <Key />
            أمّن اسمك من «حسابي»
          </button>
        </div>
