 ...
 * اللعبةُ تعرضها على شاشة البدء، والخادمُ وحدَه يحكم بها عند كلّ جولة (`watan_finish_run`).
 */
export async function GET() {
  ...
  const [me, win] = await Promise.all([
    hash || user ? sb.rpc("watan_me", { p_token_hash: hash, p_user: user }) : Promise.resolve({ data: null, error: null }),
