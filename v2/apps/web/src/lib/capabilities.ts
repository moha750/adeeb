...
export const SECTION_CAP = {
  ...
  // **معرفة ديبو** — وقائعُه الثابتة تُكتب وتُوقَف من هنا (٢٠٢٦-٠٨-٢٢). ومفتاحُها مفتاحُ
  // سجلّه، وليس نقضًا لقاعدة «لكلّ تبويبٍ قفلُه»: تلك تمنع مفتاحًا يفتح غرفًا **مختلفة**،
  // وهاتان غرفتا موضوعٍ واحدٍ هو ديبو — سجلٌّ يُقرأ ليُعرَف أين كذبت معرفتُه، ومعرفةٌ
  // تُصحَّح بما قُرئ. ومن فصلهما بمفتاحين فصل الداءَ عن دوائه.
  "/dashboard/deebo/knowledge": "manage_deebo",
  // **طبعُ ديبو** — نبرتُه وحدودُه ومحظوراتُه (٢٠٢٦-٠٨-٢٢). ومفتاحُه مفتاحُ أختيه للعلّة
  // نفسِها: ثلاثُ غرفٍ لموضوعٍ واحد. ومن ملك تصحيحَ ما يعرف ملك ضبطَ كيف يقوله.
  "/dashboard/deebo/persona": "manage_deebo",
  // **خمّن الكلمة** — غرفةُ اللعب. قدرتُها `manage_games` قائمةٌ في القاعدة منذ بناء
  // اللعبة (٢٠٢٦-٠٥-١٠) ولم يكن لها في V2 باب، وحارسُها `gw_is_admin` يقرؤها اليوم
  // بـ`check_user_permission` بعد أن أُعدم منه الترقيم.
  "/dashboard/games": "manage_games",
  // **بنكُ الكلمات** — مفتاحُه مفتاحُ غرفته، على سابقة ثلاثيّ ديبو: تلك القاعدة تمنع
  // مفتاحًا يفتح غرفًا **مختلفة**، وهاتان غرفتا موضوعٍ واحد — كلماتٌ تُكتَب، وغرفةٌ
  // تُلعَب بها. ومن ملك أن يُدير اللعبةَ ملك أن يختار ما يُلعَب به.
  "/dashboard/games/words": "manage_games",
  ...
  // **عينُ النادي على ملصقاته** — قفلٌ مستقلٌّ عن قفل الأداة: من يصنع باركوده ليس بالضرورة
  // من يُشرف على باركودات الناس. والقدرةُ يحملها دورُ «أَدِيب» (ترحيل ٢٠٢٦-٠٩-٠٥).
  "/dashboard/tools/qr/oversight": "oversee_qr",
