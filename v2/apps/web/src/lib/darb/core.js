...
// @ts-nocheck
/**
 * **لبُّ «دربك خضر» — مولَّدٌ من اللعبة نفسِها، فلا يُحرَّر هنا بيد.**
 *
 ...
 *
 * **النسخة `bb585ffc5310`** بصمةُ هذا النصّ (sha256، أوّلُ ١٢ خانة). واللعبةُ ترسل بصمتَها
 ...
 *
 * والتوليدُ: `python3 tools/site.py <v2/apps/web>` في حزمة اللعبة، يبني معه `public/games/darbak-khadar/game.html`
...
/* ── مُدَدُ الحركةِ وخزنُ الضغطة ──────────────────────────────────
   ...
   ولا شيءَ هنا غيرُ + − × ÷.
   ومدّتاهما أطولُ منذ ٢٠٢٦-٠٩-٢٥ (ملاحظةُ اللاعبين قبل المسابقة: «القفزُ والانزلاقُ
   قصيران ولا أقدرُ أوزنُهما»): القفزةُ ٠٫٧٠ث (كانت ٠٫٥٢) والانزلاقُ ٠٫٦٠ث (كان ٠٫٤٢)،
   فتتّسعُ نافذةُ التوقيتِ قرابةَ النصف. والمولِّدُ يقرأُ المدّتين نفسَيهما (spanOf)،
   فلا يُطلَبُ من اللاعبِ ما لا يُستطاع. */
var JUMP_D=0.70, SLIDE_D=0.60, BUF_W=0.20;
var WORDS=[["و","ط","ن"],["ر","ؤ","ي","ة"],["ك","ر","م"],["ه","و","ن"]];
/* ── الخصائصُ والمصّاص ─────────────────────────────────────────────────────
   ...
   والحروفُ لم تعد نقاطًا: كلُّ كلمةٍ تكتملُ تمنحُ خاصيّةً ١٥ ثانية، والكلمةُ من معنى خاصيّتِها:
     ...
     كرم   ← مغناطيس  يجودُ عليك بالحروفِ والأكوابِ من المساراتِ الثلاثة
     هون   ← تمهّل    يبطؤ الجريُ ٢٨٪ فتمشي على هونك
...
Sim.prototype.input=function(a){
  ...
  if(a===1||a===2){
    ...
    /* الحركةُ الجديدةُ مشروعةٌ لحظةَ يلمسُ الأرضَ في الرسم (air≤0.12)، لا بعدَ نهايةِ القفزةِ كاملة.
       فآخرُ ٠٫١٢ث من القفزةِ يُعَدُّ على الأرضِ في الاصطدام (low يحتاجُ air>0.12)، وكانت القفزةُ
       الجديدةُ لا تُسمَحُ فيه: لحظةٌ يرى فيها اللاعبُ نفسَه هابطًا، مكشوفًا للصبّةِ ولا يملكُ القفز.
       فالصبّتان المتتاليتان كانتا اصطدامًا مهما أحسن (ملاحظةُ محمد ٢٠٢٦-٠٩-٢٥). وكذلك آخرُ ٠٫٠٨ث
       من الانزلاق. والحركةُ الجديدةُ تُصفِّرُ الأخرى فلا يجتمعان. */
    if(s.air<=0.12&&s.slide<=0.08){ if(a===3){ s.air=JUMP_D; s.slide=0; } else { s.slide=SLIDE_D; s.air=0; } }
    /* الأحدثُ يغلبُ الأقدم، ويبقى محفوظًا حتّى تنتهيَ الحركةُ القائمة (قرارُ محمد ٢٠٢٦-٠٩-٢٥: «قفزةٌ
       تاليةٌ عند الهبوط»): اضغط القفزَ في الهواء تقفزْ ثانيةً لحظةَ تلمسُ الأرض، وكذلك الانزلاق.
       وكانت تُحفَظُ BUF_W وحدَها فتضيعُ ضغطةُ منتصفِ الهواء. */
    else { s.bufA=a; s.bufT=Math.max(s.air,s.slide)+BUF_W; }
...
Sim.prototype.tick=function(){
  ...
  if(s.bufA>0){
    if(s.air<=0.12&&s.slide<=0.08){
      if(s.bufA===3){ s.air=JUMP_D; s.slide=0; } else { s.slide=SLIDE_D; s.air=0; }
      s.bufA=0; s.bufT=0;
    } else {
      s.bufT=s.bufT-STEP;
...
export const CORE_VERSION = "bb585ffc5310";
