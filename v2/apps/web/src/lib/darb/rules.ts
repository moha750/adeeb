/**
 * **قواعدُ «دربك خضر» الخالصة** — ما يحكم به الخادمُ على الجولة وعلى الاسم.
 *
 * خالصٌ عمدًا: لا قاعدةَ ولا كوكيزَ ولا شبكة. فيُختبَر وحدَه (`__tests__/rules.test.ts`)،
 * ويُنادى من أبواب الخادم في `app/games/darbak-khadar/api/*`.
 *
...
/**
 * **لا حدَّ للجولة في اللعب، وحدُّ الخادم عمرُ التذكرة نفسُه: ثلاثُ ساعات.**
 *
 * كان الحدُّ ربعَ ساعةٍ تُنهي اللعبةُ الجولةَ عنده («بلغتَ آخرَ الطريق!») ويُحجَز ما بلغه للمراجعة،
 * على ظنّ أنّ إنسانًا لا يصمدها. فصمدتها لاعبةٌ في ليلة الافتتاح (٥٠٧٠٤ م في ١٥٫٧ دقيقةً حقيقيّة)،
 * واعتُمدت جولتُها، ورفع المالكُ الحدَّ (٢٠٢٦-٠٩-٢٦: «اجعل اللعبة بلا حد ١٥ دقيقة»).
 * والسرعةُ تقف عند ٦٢ م/ث فلا تنتهي الجولةُ بذاتها. فالسقفُ هنا ما لا تبلغه جولةٌ صالحة: التذكرةُ
 * تبطل بعد ثلاث ساعات (`darb_finish_run`)، فما طال عنها باطلٌ قبل أن يُعاد. والإعادةُ عنده نصفُ ثانية.
 */
export const MAX_TICKS = 3 * 60 * 60 * 60;

/** سقفُ الضغطات في جولة: ثلاثٌ في الثانية على طول الحدّ كلِّه، وفوقه ليس لعبًا. */
export const MAX_PRESSES = 3 * 60 * 60 * 3;
...
export type Replay = {
  ...
  dist: number;
  candies: number;
  /** بلغ حدَّ الخادم حيًّا (ثلاثَ ساعات لعب): جولةٌ تُحجَز للمراجعة، ولا تبلغه جولةٌ صالحة. */
  capped: boolean;
};
