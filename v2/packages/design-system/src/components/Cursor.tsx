...
export function Cursor({ scopeRef, className }: CursorProps) {
  ...
  return (
    <div ref={rootRef} className={cn("cur", className)} data-on="false" data-mode="idle" aria-hidden>
      ...
      {/* **الغلافُ يحمل الحركة والريشةُ تحمل الرسم** — فصلٌ مقصود: الدورانُ على
          عنصرٍ HTML تُرفَع طبقتُه إلى المعالج الرسوميّ فتُدار بلا إعادة رسم؛ ولو
          أُديرت الـSVG نفسُها لأُعيد رسمُ مسارها وحوافّها المنعّمة كلَّ إطار،
          وحدُّها شعرةٌ بيضاء 1.4px — أشدُّ ما يزحف عند الدوران.
          والمقاسُ من `--nib` في المكتبة لا من سِمتَي العنصر. */}
