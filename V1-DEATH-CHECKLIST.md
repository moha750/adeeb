...
|---|---|
| ١ · ٢ (`role_id` وتريغراه) | ⛔ **يمنعه V2** — تبويب الصلاحيات ومقعد المعاينة يقرآنه ويكتبانه |
...
### ١) `user_roles.role_id` + `role_permissions.role_id` — ⛔ **صار V2 هو المانع**

**الشكل:** مفتاح مزدوج (رقم + اسم) يزامنه تريغر `sync_role_key`.
**كان يمنعه:** `admin/dashboard.js` في ٨٢ موضعًا — **مات**.
**يمنعه الآن (جرد 2026-08-01):** V2 نفسه يقرؤه ويكتبه:

- `dashboard/system/permissions/actions.ts` — `role_permissions.upsert({ role_id, permission_id })` و`delete().eq("role_id", …)`
- `dashboard/system/permissions/data.ts` — `select("role_id, permission_id")` و`user_roles.select("user_id, role_id")` (٣ مواضع)
- `lib/preview-seat.ts` — يكتب `role_id` في مقعد المعاينة

> **الشرط:** تُرحَّل هذه المواضع إلى `role_key` أوّلًا (وهو موجودٌ ومزامَنٌ سلفًا)، ثمّ يُسقَط العمود. وإسقاطه قبلها **يكسر تبويب الصلاحيات و«العرض بصفة»** حيًّا.
