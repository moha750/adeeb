...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ===================== Testimonials (آراء الأعضاء) - Dynamic Fetch & Render =====================
  function timeAgoAr(iso){
    try {
      ...
      if (week < 5) {
        if (week === 1) return 'منذ أسبوع';
        if (week === 2) return 'منذ أسبوعين';
        return `منذ ${week} أسابيع`;
      }
      const month = Math.floor(day / 30);
      if (month < 12) {
        if (month === 1) return 'منذ شهر';
        if (month === 2) return 'منذ شهرين';
        return `منذ ${month} أشهر`;
      }
      const year = Math.floor(day / 365);
      if (year === 1) return 'منذ سنة';
      if (year === 2) return 'منذ سنتين';
  ...
  function getLocalTestimonials(){
    try { const raw = localStorage.getItem('adeeb_testimonials'); const arr = raw ? JSON.parse(raw) : []; return Array.isArray(arr) ? arr : []; } catch { return []; }
  }

  function setLocalTestimonials(arr){
    try { localStorage.setItem('adeeb_testimonials', JSON.stringify(Array.isArray(arr) ? arr : [])); } catch {}
  }
  ...
  async function fetchTestimonials(){
    try {
      ...
      if (sb){
        const { data, error } = await sb
          .from('testimonials')
          .select('id, author_name, committee, rating, content, avatar_url, visible, created_at, order')
          .eq('visible', true)
          .order('order', { ascending: true, nullsFirst: true })
          .order('created_at', { ascending: false });
        if (error) throw error;
    ...
    } catch (e){ console.warn('testimonials fetch failed, will try localStorage.', e); }
  ...
  function buildStars(r){
    ...
    let html = '';
    for (let i=0;i<5;i++) html += i < n ? '<i class="fa-solid fa-star"></i>' : '<i class="fa-regular fa-star"></i>';
    return html;
  }

  function renderTestimonials(list){
    const wrap = document.querySelector('.testimonials-swiper .swiper-wrapper');
    if (!wrap) return;
    wrap.innerHTML = '';
    (list||[]).forEach((row)=>{
      const slide = document.createElement('div');
      slide.className = 'swiper-slide';
      slide.setAttribute('data-rating', String(row.rating || 0));
      if (row.committee) slide.setAttribute('data-committee', String(row.committee));
      const name = (row.author_name || '').toString();
      const committee = (row.committee || '').toString();
      const img = (row.avatar_url || '').toString();
      const dateLabel = timeAgoAr(row.created_at || new Date().toISOString());
      slide.innerHTML = `
        <div class="testimonial-card">
          <div class="testimonial-header">
            <div class="testimonial-stars" aria-label="${Number(row.rating||0)} من 5">${buildStars(row.rating)}</div>
            <div class="testimonial-date">${dateLabel}</div>
          </div>
          <i class="fas fa-quote-right quote-bg"></i>
          <p class="testimonial-text">${(row.content || '').toString().replace(/</g,'&lt;').replace(/>/g,'&gt;')}</p>
          <div class="testimonial-user">
            ${img ? `<img src="${img}" alt="عضو" loading="lazy" />` : `<img src="https://i.pravatar.cc/96?u=${encodeURIComponent(name||'user')}&img=1" alt="عضو" loading="lazy" />`}
            <div>
              <strong>${name || 'عضو'}</strong>
  ...
  async function loadTestimonialsSection(){
    try {
      ...
      // ignore, fallback to static markup
    } finally {
      // Whether dynamic or static, (re)initialize behaviors
  ...
  // Initialize testimonials on load (dynamic first; fallback to static)
