...
```
١. أضِف user_roles.role_name  + role_permissions.role_name   (بلا FK — انظر أدناه)
٢. املأهما من role_id
٣. تريغر يزامن العمودين في كلّ INSERT/UPDATE  ← الثمن الحقيقيّ الوحيد والحارس
...
> ### تصحيح: لا FK على `role_name` — المفتاح الثاني يكسر V1
>
> نصّت الخطوة ١ على `FK → roles(role_name)`. **جُرّب فانكسر V1 كسرًا حيًّا، فأُسقط.**
>
> PostgREST يستنتج «التضمين» من المفاتيح الأجنبيّة وحدها. وبمفتاحَين من `user_roles` إلى `roles` صار كلّ تضمينٍ قائم ملتبسًا:
>
> ```
> GET /user_roles?select=user_id,roles(role_level)
> → PGRST201: Could not embed because more than one relationship
>             was found for 'user_roles' and 'roles'
> ```
>
> وذلك **٢٨ موضعًا في ١٢ ملفًّا** من V1 الحيّ — منها فحص وصول الأدمن نفسه (`admin/js/master-access.js:31`) و٥ دوالّ حافّة. أي أنّ الخطوة ١ تنقض الخطوة ٤ حرفيًّا: «V1 لا يشعر بشيء».
>
> **العمود إضافيّ صرف؛ الكاسر هو المفتاح الثاني إلى الجدول نفسه — لا العمود.**
>
> ولا حارس يضيع: `sync_role_key` **يشتقّ** الاسم من `roles`، فالقيمة الباطلة غير قابلة للتمثيل أصلًا (اسمٌ لا دور له ⇒ `23503`، ومفتاحان متعارضان ⇒ `23514` — مُثبَتان بالتجربة بعد إسقاط المفتاحين). وحذف الدور يُسقِط الصفّ عبر `role_id FK (CASCADE)` فيذهب الاسم مع صفّه. فمكسب FK ≈ صفر وثمنه كسر الحيّ.
